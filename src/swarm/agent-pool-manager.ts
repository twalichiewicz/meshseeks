...
export interface AgentPoolConfig {
  ...
  cooldownMs: number;             // Cooldown between scaling events
}

/**
 * Agent request in the queue.
 */
...
export class AgentPoolManager extends EventEmitter {
  ...
  private agents: Map<string, SwarmAgentConfig> = new Map();
  ...
  private targetAgentCount: number;
   ...
   * Create a new agent.
   */
  private async createAgent(role?: ExtendedAgentRole): Promise<string> {
    const agentId = this.generateAgentId();

    const agent: SwarmAgentConfig = {
      id: agentId,
   ...
   */
  private findAvailableAgent(preferredRole?: ExtendedAgentRole): SwarmAgentConfig | undefined {
   ...
   */
  private insertRequestByPriority(request: AgentRequest): void {
  ...
  private processQueue(): void {
    while (this.requestQueue.length > 0) {
      ...
      try {
        const agentId = this.assignAgentToTask(
          availableAgent.id,
          request.role,
          request.taskId
        );
  ...
  private checkScaleUp(): void {
    if (this.isScaling || this.isShuttingDown) return;
    ...
    if (queueDepth >= this.config.scaleUpThreshold) {
      ...
      const neededAgents = Math.min(
        queueDepth - idleAgents,
  ...
  private checkScaleDown(): void {
    ...
    const excessIdleAgents = idleAgents.filter(
      a => now - a.lastActivityAt > this.config.scaleDownThreshold
    );

    // Only scale down if above minimum
    const agentsToRemove = Math.min(
      excessIdleAgents.length,
   ...
   */
  async scaleUp(count: number, reason: string): Promise<void> {
    if (this.isScaling || this.isShuttingDown) return;
   ...
   */
  async scaleDown(count: number, reason: string): Promise<void> {
    if (this.isScaling || this.isShuttingDown) return;
   ...
   */
  async setTargetAgentCount(count: number, reason: string = 'Manual scaling'): Promise<void> {
  ...
  getStats(): PoolStats {
    ...
    let scalingDecision: 'scale_up' | 'scale_down' | 'maintain' = 'maintain';
   ...
   */
  private getIdleAgentCount(): number {
   ...
   * Get all agents.
   */
  getAgents(): Map<string, SwarmAgentConfig> {
   ...
   * Get agent by ID.
   */
  getAgent(agentId: string): SwarmAgentConfig | undefined {
