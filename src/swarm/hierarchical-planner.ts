...
/**
 * Planning context passed to decomposition.
 */
...
/**
 * Decomposition strategy determines how to break down tasks.
 */
export type DecompositionStrategy =
  ...
  | 'phased';        // Tasks grouped into phases

/**
 * Planning instruction for the planner agent.
 */
...
export interface PlannerConfig {
  ...
  autoDecomposeThreshold: number;  // Complexity threshold for auto-decomposition
  minTasksForDecomposition: number;
  enableSubPlanners: boolean;
}
...
export class HierarchicalPlanner {
  ...
  async decompose(
    ...
    const effectiveInstruction: PlanningInstruction = {
    ...
    const templates = this.analyzeAndPlan(
      parentTask,
    ...
    const tasks = this.createTasksFromTemplates(
      templates,
      parentTask,
      context,
      effectiveInstruction.strategy
    );
  ...
  private analyzeAndPlan(
    ...
    const maxTasks = Math.min(
      instruction.maxTasks || this.config.maxTasksPerLevel,
      context.maxTasksPerLevel
    );
   ...
   */
  private planFeatureImplementation(
    task: HierarchicalTask,
    instruction: PlanningInstruction
  ): TaskTemplate[] {
    const templates: TaskTemplate[] = [];
   ...
   */
  private planTestingSuite(
    task: HierarchicalTask,
    _instruction: PlanningInstruction
  ): TaskTemplate[] {
    const templates: TaskTemplate[] = [];
    const basePrompt = task.prompt;
  ...
  private calculateDependencies(
    index: number,
    previousTasks: HierarchicalTask[],
    template: TaskTemplate,
    strategy: DecompositionStrategy,
    parentId: string
  ): string[] {
    // Always include parent as dependency
    const dependencies: string[] = [parentId];
    ...
    switch (strategy) {
      ...
      case 'hybrid':
        ...
        if (template.role === 'implementation') {
          ...
          const implTasks = previousTasks.filter(t =>
            t.role === 'implementation' || t.role === 'testing'
          );
          dependencies.push(...implTasks.map(t => t.id));
        }
        break;
      ...
      case 'phased':
        ...
        const phases: ExtendedAgentRole[][] = [
          ['analysis', 'planner'],
   ...
   */
  private generateTaskId(sessionId: string, parentId: string): string {
    this.taskIdCounter++;
   ...
   */
  estimateComplexity(task: HierarchicalTask): number {
    let complexity = 0;
    const prompt = task.prompt.toLowerCase();
   ...
   */
  shouldAutoDecompose(task: HierarchicalTask, context: PlanningContext): boolean {
    ...
    const leafRoles: ExtendedAgentRole[] = ['testing', 'documentation', 'debugging'];
  ...
  getTreeStats(
    ...
    byRole: Record<ExtendedAgentRole, number>;
    maxDepth: number;
    leafCount: number;
  } {
    const tree = taskTree instanceof Map
    ...
    const stats = {
      ...
      byRole: {} as Record<ExtendedAgentRole, number>,
      maxDepth: 0,
      leafCount: 0
    };
  ...
  private getRoleEmoji(role: ExtendedAgentRole): string {
    const emojis: Record<ExtendedAgentRole, string> = {
      analysis: '🔍',
      implementation: '⚙️',
      testing: '🧪',
      documentation: '📝',
...
if (isMainModule) {
  (async () => {
    ...
    totalTests++;
    try {
      const simpleTask: HierarchicalTask = {
        id: 'task-1',
        parentId: null,
        depth: 0,
        children: [],
      ...
      const complexTask: HierarchicalTask = {
        ...simpleTask,
        id: 'task-2',
    ...
    totalTests++;
    try {
      const featureTask: HierarchicalTask = {
        id: 'task-root',
        parentId: null,
        depth: 0,
        children: [],
    ...
    totalTests++;
    try {
      const bugTask: HierarchicalTask = {
        id: 'task-bug',
        parentId: null,
        depth: 0,
        children: [],
    ...
    totalTests++;
    try {
      const deepTask: HierarchicalTask = {
        id: 'task-deep',
        parentId: 'parent',
    ...
    totalTests++;
    try {
      const taskTree = new Map<string, HierarchicalTask>();
