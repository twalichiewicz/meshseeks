...
class MeshPerformanceTestSuite {
    ...
    totalTests = 0;
    performanceResults = [];
    async createPerformanceTestProject() {
        ...
        for (const dir of dirs) {
            ...
            for (let i = 0; i < 10; i++) {
                await fs.writeFile(join(testDir, dir, `file${i}.js`), this.generateComplexFileContent(dir, i));
            }
        }
    ...
    generateComplexFileContent(dir, index) {
        return `
...
`;
    ...
    async testBasicConcurrencyPerformance() {
    ...
    async testDependencyResolutionPerformance() {
    ...
    async simulateParallelExecution(tasks, concurrency) {
        const startTime = performance.now();
        ...
        for (const batch of batches) {
            await Promise.all(batch.map(() => this.simulateTaskExecution()));
        }
        const endTime = performance.now();
    ...
    async simulateScaleTest(agents, tasks) {
        const startTime = performance.now();
    ...
    generateDependencyPattern(complexity) {
        const tasks = [];
    ...
    async measureDependencyResolution(tasks) {
        const startTime = performance.now();
        // Simulate dependency resolution algorithm
        const resolved = new Set();
        const remaining = [...tasks];
        while (remaining.length > 0) {
            const ready = remaining.filter(task => !task.dependencies || task.dependencies.every((dep) => resolved.has(dep)));
            if (ready.length === 0) {
    ...
    async simulateTaskBatch(tasks, agents) {
    ...
    generateComplexityPrompt(complexity) {
        const prompts = {
        ...
        return prompts[complexity] || prompts.medium;
    }
