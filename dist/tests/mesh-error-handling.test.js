...
class MeshErrorHandlingTestSuite {
    ...
    async testInvalidTaskStructures() {
    ...
    async testInvalidWorkingDirectories() {
    ...
    async testConcurrencyLimitExceeded() {
    ...
    async testAgentExecutionFailures() {
    ...
    async validateWorkingDirectory(path) {
    ...
    simulateConcurrencyLimits(tasks, maxConcurrency) {
        return {
            concurrent: tasks.slice(0, maxConcurrency),
            queued: tasks.slice(maxConcurrency)
        };
    }
    simulateAgentFailure(scenario) {
        return {
            success: false,
    ...
    simulateResourceConstraint(task, resourceType) {
        const constraints = {
    ...
    simulateNetworkFailure(scenario) {
        return {
            error: scenario.error,
