...
class MeshEnhancedServer {
     ...
     */
    async handleAnalyzeProblem(args) {
    ...
    async handleExecuteTasks(args) {
        ...
        const results = await this.meshCoordinator.executeMeshTasks(typedArgs.tasks);
...
╚═══════════════════════════════════════════════╝`
     ...
     */
    async executeParallelExploration(tasks, workFolder) {
     ...
     */
    async executeIterativeRefinement(tasks, workFolder) {
        const allResults = [];
     ...
     */
    async synthesizeResults(results, originalPrompt, returnSummary) {
...
`;
     ...
     */
    generateTaskRecommendation(tasks) {
...
╚═══════════════════════════════════════════════╝`
     ...
     * Spawn agent asynchronously
     */
    async spawnAgentAsync(agentId, args, workFolder, session) {
        try {
            const task = {
                id: args.taskId,
                prompt: args.prompt,
    ...
    async handleAgentStatus(args) {
        ...
        const statusMessages = activeAgents.map(agent => `  ${agent.status === 'running' ? '⚡' : '⏳'} ${agent.taskId}: ${agent.status} (${agent.runningTime})`).join('\n');
        const response = {
...
╚═══════════════════════════════════════════════╝`
     ...
     */
    async handleCollectResult(args) {
        const typedArgs = args;
...
╚═══════════════════════════════════════════════╝`
     ...
     */
    createQuickPlan(prompt, workFolder) {
        return [
    ...
    createBalancedPlan(prompt, workFolder) {
        return [
    ...
    createThoroughPlan(prompt, workFolder) {
     ...
     */
    async handleSwarmCreateSession(args) {
     ...
     */
    async handleSwarmResumeSession(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
        ...
        const session = await this.swarmOrchestrator.resumeSession(typedArgs.sessionId, typedArgs.checkpointId, typedArgs.resetFailedTasks);
        const response = {
            status: 'success',
     ...
     */
    async handleSwarmPauseSession(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
        ...
        const session = await this.swarmOrchestrator.pauseSession(typedArgs.sessionId, typedArgs.reason);
        const response = {
            status: 'success',
     ...
     */
    async handleSwarmSessionStatus(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
     ...
     */
    async handleSwarmPlanHierarchical(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
     ...
     */
    async handleSwarmVerifyTask(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
     ...
     */
    async handleSwarmScaleAgents(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
     ...
     */
    async handleSwarmCreateCheckpoint(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
        ...
        const checkpointId = await this.swarmOrchestrator.createCheckpoint(typedArgs.sessionId, typedArgs.description);
        const response = {
            status: 'success',
     ...
     */
    async handleSwarmListCheckpoints(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        if (!typedArgs?.sessionId) {
            throw new McpError(ErrorCode.InvalidParams, 'Missing required parameter: sessionId');
     ...
     */
    async handleSwarmRestoreCheckpoint(args) {
        await this.ensureSwarmInitialized();
        const typedArgs = args;
        ...
        const session = await this.swarmOrchestrator.restoreCheckpoint(typedArgs.sessionId, typedArgs.checkpointId);
        const response = {
            status: 'success',
