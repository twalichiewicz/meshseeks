...
export class CheckpointStore {
    ...
    constructor(baseDir = '~/.meshseeks/sessions', maxCheckpointsPerSession = 100) {
     ...
     */
    async createCheckpoint(session, agentStates, contextStore, options) {
        this.ensureInitialized();
        const checkpointId = this.generateCheckpointId();
        const timestamp = Date.now();
        // Convert Maps to Records for serialization
        const taskTreeData = session.taskTree instanceof Map
            ? Object.fromEntries(session.taskTree)
            : session.taskTree;
        const agentStatesData = agentStates instanceof Map
            ? Object.fromEntries(agentStates)
            : agentStates;
        const contextStoreData = contextStore instanceof Map
        ...
        // Determine file path
        const checkpointPath = path.join(session.id, SESSION_DIRS.checkpoints, `${checkpointId}.json${options.compress ? '.gz' : ''}`);
        // Write checkpoint
     ...
     */
    async listCheckpoints(sessionId, limit, offset) {
        this.ensureInitialized();
        ...
        for (const file of paginatedFiles) {
            ...
            const checkpoint = file.name.endsWith('.gz')
                ? await this.backend.readCompressedJSON(filePath)
                : await this.backend.readJSON(filePath);
            if (checkpoint) {
     ...
     * Save a task result.
     */
    async saveTaskResult(sessionId, taskId, result) {
        this.ensureInitialized();
     ...
     * Load a task result.
     */
    async loadTaskResult(sessionId, taskId) {
        this.ensureInitialized();
        const resultPath = path.join(sessionId, SESSION_DIRS.results, `${taskId}.json`);
        return this.backend.readJSON(resultPath);
    }
    // ===========================================================================
    // UTILITIES
    // ===========================================================================
    ...
    (async () => {
        ...
        const mockSession = {
            ...
            metrics: {
                ...
                tasksByStatus: { completed: 1, pending: 1 },
                tasksByRole: { planner: 1, implementation: 1 }
            },
            errors: []
        };
        try {
        ...
        try {
            const result = await store.createCheckpoint(mockSession, {}, {}, {
                trigger: 'manual',
