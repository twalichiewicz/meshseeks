...
export class FileBackend {
     ...
     */
    async writeJSON(relativePath, data, pretty = true) {
        const fullPath = this.getFullPath(relativePath);
     ...
     */
    async writeCompressedJSON(relativePath, data) {
        const fullPath = this.getFullPath(relativePath);
        const tempPath = `${fullPath}.tmp.${Date.now()}`;
        try {
            await fs.mkdir(path.dirname(fullPath), { recursive: true });
     ...
     */
    async readCompressedJSON(relativePath) {
        const fullPath = this.getFullPath(relativePath);
        try {
            const chunks = [];
            await pipeline(createReadStream(fullPath), createGunzip(), async function* (source) {
    ...
    async listFilesRecursive(fullPath, relativePath, results, recursive) {
     ...
     */
    async calculateChecksum(relativePath) {
        const fullPath = this.getFullPath(relativePath);
     ...
     */
    async getDirectorySize(relativePath) {
     ...
     */
    async cleanup(relativePath, options) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.isDirectory)
                continue;
            const shouldDelete = (options.maxAge && now - file.modifiedAt > options.maxAge) ||
    ...
    (async () => {
        ...
        try {
            ...
            if (!writeResult.success) {
                ...
                const readData = await backend.readJSON('test.json');
        ...
        try {
            ...
            if (!writeResult.success) {
                ...
                const readData = await backend.readCompressedJSON('test.json.gz');
