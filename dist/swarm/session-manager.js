...
export class SessionManager extends EventEmitter {
    ...
    agentStates = new Map();
    contextStores = new Map();
    autoCheckpointTimers = new Map();
    sessionExpiryTimers = new Map();
    ...
    async createCheckpoint(sessionId, options) {
        ...
        const result = await this.checkpointStore.createCheckpoint(session, agentStates, contextStore, options);
     ...
     * Update agent state.
     */
    updateAgentState(sessionId, agentId, updates) {
        const agents = this.agentStates.get(sessionId);
        if (!agents)
            return;
     ...
     * Emit a swarm event.
     */
    emitEvent(type, sessionId, data, taskId, agentId) {
        const event = {
            type,
            timestamp: Date.now(),
            sessionId,
    ...
    createInitialMetrics() {
        return {
            ...
            tasksByRole: { planner: 1 }
        };
    }
    /**
     * Update metrics when task status changes.
     */
    updateMetricsForStatusChange(metrics, previousStatus, newStatus) {
