...
export class JudgeSystem {
    ...
    async verify(request) {
        ...
        for (const criterionConfig of criteriaToEvaluate) {
            const criterion = await this.evaluateCriterion(task, result, criterionConfig, request.customPrompt);
     ...
     */
    selectCriteria(task, customCriteria) {
     ...
     */
    async evaluateCriterion(task, result, criterionConfig, customPrompt) {
     ...
     */
    ruleBasedEvaluation(task, result, criterionType) {
        const evidence = [];
        let score = 0;
        let feedback = '';
    ...
    evaluateCompleteness(task, result, evidence) {
        let score = 0;
    ...
    evaluateCorrectness(task, result, evidence) {
        let score = 0;
        // No errors reported
        if (!result.error) {
            score += 0.2;
    ...
    evaluateQuality(result, evidence) {
        let score = 0;
    ...
    evaluateTesting(result, evidence) {
        let score = 0;
    ...
    evaluateDocumentation(result, evidence) {
        let score = 0;
        const output = result.output || '';
    ...
    evaluateSecurity(result, evidence) {
    ...
    evaluatePerformance(result, evidence) {
        let score = 0;
     ...
     */
    generateReworkInstructions(task, criteria) {
     ...
     */
    createReworkInstruction(task, verdict, attemptNumber) {
     ...
     */
    buildReworkPrompt(task, failedCriteria) {
